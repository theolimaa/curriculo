...
export default function Formulario() {
  ...
  const handleGenerateCV = async () => {
    setStep("loading");
    try {
      const res = await fetch("/api/generate-cv", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId }) });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto", padding: "48px 24px 80px" }}>
        ...
        {step === "form" && (
          <div className="fade-up">
            ...
            <div style={{ display: "", TemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
